module EventCalendar
  module CalendarHelper
    ...
    def event_content(event, days, cur_offs, idx, event_width, event_height, event_margin, &block)
      cal = ""
      cal << "<div event_id='#{event.id}' day='#{event.start_at.day}' color='#{event.category.color}' "
      cal << "class='event event_#{event.id}' "
      cal << "style='background: #{event.category.color}; width: #{event_width*days}px; height: #{event_height}px; top: #{idx*(event_height+event_margin)+event_margin}px; left:#{cur_offs}px;' "
